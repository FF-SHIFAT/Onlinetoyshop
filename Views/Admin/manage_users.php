...
<head>
    ...
    <style>
        ...
        .modal { display: none; position: fixed; z-index: 1100; left: 0; top: 0; width: 100%; height: 100%; background-color: rgba(0,0,0,0.5); overflow-y: auto; }
...
<body>
    ...
    <div class="admin-container">
        <div class="sidebar">
            ...
            <a href="manage_users.php" class="active">Registered Users</a>
        </div>
        ...
        <div class="main-content">
            ...
            <table class="data-table" style="width:100%; border-collapse:collapse; background:white; box-shadow: 0 4px 10px rgba(0,0,0,0.05);">
                <thead style="background:#343a40; color:white;">
                    <tr><th style="padding:12px;">ID</th><th>Name</th><th>Email</th><th>Phone</th><th>Address</th><th>Action</th></tr>
                </thead>
    ...
    <div id="userModal" class="modal">
        <div class="modal-content">
            ...
            <form action="../../Controllers/userControl.php" method="POST" id="userForm">
                ...
                <div class="form-group"><label>Full Name</label><input type="text" name="name" id="u_name" required></div>
                <div class="form-group"><label>Email</label><input type="email" name="email" id="u_email" required></div>
                <div class="form-group"><label>Phone</label><input type="text" name="phone" id="u_phone" required></div>
                <div class="form-group"><label>Role</label><select name="role" id="u_role"><option value="customer">Customer</option><option value="admin">Admin</option></select></div>
                <div class="form-group"><label>Address</label><textarea name="address" id="u_address" rows="2"></textarea></div>
                <div class="form-group"><label>Password <small style="color:red;">(Leave blank to keep current)</small></label><input type="password" name="password" id="u_pass" placeholder="Enter new password"></div>
    ...
    <script>
        ...
        function loadUsers() {
            ...
            xhr.onreadystatechange = function() { if (this.readyState === 4 && this.status === 200) { document.getElementById("userTableBody").innerHTML = this.responseText; } };
        ...
        function viewUser(id) {
            ...
            xhr.onreadystatechange = function() {
                if (this.readyState === 4 && this.status === 200) {
                    ...
                    let html = "<p><strong>Name:</strong> " + data.name + "</p><p><strong>Email:</strong> " + data.email + "</p><p><strong>Phone:</strong> " + data.phone + "</p><p><strong>Role:</strong> <span style='background:#333; color:white; padding:2px 6px; border-radius:4px; font-size:12px;'>" + data.role.toUpperCase() + "</span></p>";
                    let address = data.address_line ? data.address_line : 'N/A';
                    html += "<p><strong>Address:</strong> " + address + "</p><p><strong>Joined Date:</strong> " + data.created_at + "</p>";
        ...
        function deleteUser(id) {
            if(confirm("Are you sure? This action cannot be undone!")) {
                ...
                xhr.open("POST", "../../Controllers/userControl.php", true);
                xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
                xhr.onreadystatechange = function() { if (this.readyState === 4 && this.status === 200) { if (this.responseText.trim() == "success") { loadUsers(); } else { alert("Failed to delete!"); } } };
